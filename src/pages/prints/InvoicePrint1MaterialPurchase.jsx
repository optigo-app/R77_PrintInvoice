...
const InvoicePrint1MaterialPurchase = ({
  ...
  const totalDiscountAMT = Array.isArray(finalD)
    ...
    const totalTaxAmountjson3 = json3Data.reduce((sum, item) => {
  ...
  //      json0Data?.GSTProfileid == 0 ? (taxprofile1 || 0) : (totalTaxAmountjson3 || 0) +
  //     (addLess || 0)  ) - totalDiscountAMT;

  const GrandTotal =
  (totalAmount || 0) +
  (
    json0Data?.GSTProfileid == 0
      ? (taxprofile1 || 0)
      : (totalTaxAmountjson3 || 0)
  ) +
  (addLess || 0) -
  (totalDiscountAMT || 0);
...
//  json0Data?.GSTProfileid == 1 ?{
//    return Object.entries(extraTaxAmont || {})
//   .filter(([key]) => key.includes("TotalAmount") && key !== "GSTTotalAmount" && key !== "totaltaxAmount")
//   .map(([key, value]) => ({
//     TaxName: key.replace("TotalAmount", ""), // CGST, SGST, IGST
//     TaxAmount: value || 0
//   }))
...
//         )
//         .map(([key, value]) => ({
//           TaxName: key.replace("TotalAmount", ""),
//           TaxAmount: value || 0
//         }))
//     : Object.values(
//         (extraTaxAmont || []).reduce((acc, item) => {
...
//       );
      ...
      // if (json0Data?.GSTProfileid == 1) {
      //   // from setJson3Data (array format)
      ...
      useEffect(() => {
        ...
        if (json0Data?.GSTProfileid == 1) {
          ...
          total =
            (data?.CGSTTotalAmount || 0) +
            (data?.SGSTTotalAmount || 0) +
            (data?.IGSTTotalAmount || 0);
        }
        ...
        if (json0Data?.GSTProfileid == 0) {
          ...
          for (let i = 1; i <= 5; i++) {
            total += data?.[`tax${i}Amount`] || 0;
          }
        }
      
        setTaxprofile1(total);
          ...
          <div className="container_inv2">
            ...
            {addLess != 0 && (
              <div className="disflx spfntbH">
                ...
                <div className="taxwdthlab spbrlFt spbrRht" style={{ width: "85.6%" }}></div>
                <div className="taxwdth1lab spbrRht" style={{ width: "19.7%" }}>
                  <p className="spfntBld"> {addLess > 0 ? "Add" : "Less"} </p>
                </div>
                <div className="taxwdth2lab spbrRht" style={{ width: "14.3%" }}>
                  <p className="spacLft2 spfntBld">{formatAmount(addLess, 2)}</p>
                </div>
              </div>
            )}
