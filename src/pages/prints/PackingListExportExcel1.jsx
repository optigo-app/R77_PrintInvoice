...
import "../../assets/css/prints/packinglistexportexcel1.css";
        ...
        <>
            {loader ? (
                ...
                <div>
                    ...
                    <table id="table-to-xls" className="srt-table">

                        <tbody>
                            <tr>
                                <td colSpan={2}>EXPORT NO : </td>
                                <td> {result?.header?.InvoiceNo}</td>
                                <td colSpan={12}></td>
                                <td colSpan={1}>DATE</td>
                                <td colSpan={2}>{result?.header?.EntryDate}</td>
                            </tr>
                        </tbody>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style={th}>WSTG GMS</th>
                                <th style={th}>Gold Rate$</th>
                                <th style={th}>Gold Value $</th>
                                <th style={th}>Pcs.</th>
                                <th style={th}>STUDDING TYPE</th>
                                <th style={th}>STUDDING WT</th>
                                <th style={th}>STUDDING QUALITY</th>
                                <th style={th}>STUDDING RATE $</th>
                                <th style={th}>STUDDING VALUE $</th>
                        ...
                        <tbody>
                            {MergedData.map((group, groupIndex) => {
                                ...
                                const sectionHeaderRow = (
                                    <tr key={`section-${groupIndex}`}>
                                        <td colSpan={18} style={sectionHeaderTd}>{group?.DisplayName}</td>
                                    </tr>
                                );
                                ...
                                const itemRows = group.items.map((e, i) => {
                                    ...
                                    const diaRows = getDiamondRowsByMaterial(e); // [{materialTypeName, shapes[], pcs, wt, QualityName, rate, amount}, ...]
                                    const totalSubRows = diaRows.length || 1;
                                    ...
                                    return Array.from({ length: totalSubRows }).map((_, subIdx) => {
                                        ...
                                        const dRow = diaRows[subIdx] || null;
                                        ...
                                        return (
                                            <tr key={`item-${groupIndex}-${i}-${subIdx}`}>
                                                ...
                                                {isFirst && (
                                                    <>
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "center" })}>
                                                            {currentSr}
                                                        </td>
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "left" })}>
                                                            {e?.designno} {jobLabel}
                                                        </td>
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "center" })}>
                                                            {e?.Categoryname}
                                                        ...
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "right" })}>
                                                            {formatAmount(e?.NetWt, 3)}
                                                        </td>

                                                        {/* WSTG GMS — TODO: confirm source field */}
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "right" })}>
                                                            {formatAmount(e?.Wastage, 3)}
                                                        </td>

                                                        {/* Gold Rate$ — TODO: confirm source field */}
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "right" })}>
                                                            {formatAmount(e?.totals?.metal?.Rate, 2)}
                                                        </td>

                                                        {/* Gold Value $ — TODO: confirm source field */}
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "right" })}>
                                                             {formatAmount(e?.MetalAmount, 2)}
                                                        </td>
                                                    </>
                                                )}
                                                ...
                                                {/* ── STUDDING TYPE ── */}
                                                <td style={{ ...tdCenter, textAlign: "center" }}>
                                                    {dRow
                                                        ? dRow.materialTypeName
                                                            ? `${dRow.materialTypeName}${dRow.shapes?.length ? ` / ${dRow.shapes.join(", ")}` : ""}`
                                                            : dRow.shapes?.join(", ")
                                                        : ""}
                                                </td>
                                                ...
                                                {/* STUDDING RATE $ — TODO: confirm source field */}
                                                <td style={{ ...td, textAlign: "right" }}>
                                                    {formatAmount(dRow?.Rate, 2)}
                                                </td>

                                                {/* STUDDING VALUE $ — TODO: confirm source field */}
                                                <td style={{ ...td, textAlign: "right" }}>
                                                {formatAmount(dRow?.Amount, 2)}
                                                </td>

                                                {/* ── Labour + Grand Total — only on first sub-row, spans all ── */}
                                                {isFirst && (
                                                    <>
                                                        <td rowSpan={totalSubRows} style={tdMerged({ textAlign: "right" })}>
                                ...
                                // ── group total row ──
                                const grandDiaTotalRate = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) =>
                                        a + getDiamondRowsByMaterial(e).reduce((s, r) => s + r.Rate, 0), 0), 0);
                                const grandDiaTotalAmount = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) =>
                                        a + getDiamondRowsByMaterial(e).reduce((s, r) => s + r.Amount, 0), 0), 0);
                                ...
                                const groupGrossWt = group.items.reduce((acc, e) => acc + (e?.grosswt || 0), 0);
                                const groupNetWt = group.items.reduce((acc, e) => acc + (e?.NetWt || 0), 0);
                                const groupLabour = group.items.reduce((acc, e) => acc + (e?.MakingAmount || 0), 0);
                                const groupGrandTotal = group.items.reduce((acc, e) => acc + (e?.TotalAmount || 0), 0);

                                const totalRow = (
                                    <tr key={`total-${groupIndex}`}>
                                        <td colSpan={3} style={totalRowTdCenter}>TOTAL</td>
                                        <td style={totalRowTd}>{group?.total?.Quantity ?? group.items.length}</td>
                                        <td style={totalRowTd}></td>
                                        <td style={totalRowTd}>{formatAmount(groupGrossWt, 3)}</td>
                                        <td style={totalRowTd}>{formatAmount(groupNetWt, 3)}</td>
                                        <td style={totalRowTd}></td>
                                        <td style={totalRowTd}></td>
                                        <td style={totalRowTd}></td>
                                        ...
                                        <td style={totalRowTd}></td>
                                        <td style={totalRowTd}> {grandDiaTotalRate?.toFixed?.(2)}</td>
                                        <td style={totalRowTd}> {grandDiaTotalAmount?.toFixed?.(2)}</td>
                                        <td style={totalRowTd}>{formatAmount(groupLabour, 2)}</td>
                                        <td style={totalRowTd}>{formatAmount(groupGrandTotal, 2)}</td>
                                    </tr>
                                );

                                const spacerRow = (
                                    <tr key={`spacer-${groupIndex}`}>
                                        <td colSpan={18} style={{ border: "1px solid #000", padding: "2px" }}></td>
                                    </tr>
                                );
                            ...
                            {(() => {
                                ...
                                const grandGrossWt = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) => a + (e?.grosswt || 0), 0), 0);
                                const grandNetWt = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) => a + (e?.NetWt || 0), 0), 0);
                                const grandLabour = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) => a + (e?.MakingAmount || 0), 0), 0);
                                const grandGrandTotal = MergedData.reduce((acc, group) =>
                                    acc + group.items.reduce((a, e) => a + (e?.TotalAmount || 0), 0), 0);

                                return (
                                    <tr>
                                        <td colSpan={3} style={{ ...grandTotalTd, textAlign: "center" }}>GRAND TOTAL</td>
                                        <td style={grandTotalTd}>{grandTotal?.Quantity}</td>
                                        <td style={grandTotalTd}></td>
                                        <td style={grandTotalTd}>{formatAmount(grandGrossWt, 3)}</td>
                                        <td style={grandTotalTd}>{formatAmount(grandNetWt, 3)}</td>
                                        <td style={grandTotalTd}></td>
                                        <td style={grandTotalTd}></td>
                                        <td style={grandTotalTd}></td>
                                        ...
                                        <td style={grandTotalTd}></td>
                                        <td style={grandTotalTd}>{grandDiaTotalRate?.toFixed?.(2)}</td>
                                        <td style={grandTotalTd}>{grandDiaTotalAmount?.toFixed?.(2)}</td>
                                        <td style={grandTotalTd}>{formatAmount(grandLabour, 2)}</td>
                                        <td style={grandTotalTd}>{formatAmount(grandGrandTotal, 2)}</td>
                                    </tr>
                                );
                            })()}
                        </tbody>
                        <tbody>
                            ...
                            <tr>
                                <td colSpan={18} style={{ padding: "6px", textAlign: "right", fontWeight: "bold", fontSize: "18px" }}>
